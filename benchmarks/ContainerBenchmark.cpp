...
static void BM_Container_ResolveManyTransients(benchmark::State& state) {
  ...
  Knot::Container c;
  for (int i = 0; i < 32; ++i) c.registerService<Dummy>(TRANSIENT, i);
  for (auto _ : state) {
    for (int i = 0; i < 8; ++i) {
...
static void BM_Container_RegisterAndResolveSimpleSingleton(
...
static void BM_Container_RegisterAndResolveSimpleTransient(
    benchmark::State& state) {
  struct Dummy {
    int x;
  };
